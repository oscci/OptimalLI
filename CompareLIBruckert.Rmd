...
# round(cor(cordata[,2:7],use='complete.obs'),3)
# round(cor(cordata[,8:13],use='complete.obs'),3)
cormat.t <- round(cor(cordata[,2:13],use='complete.obs'),3)
cormat.m <- round(cor(cordata[,14:25],use='complete.obs'),3)
...
row.names(c1)<-c('frontalWG','temporalWG','parietalWG','cerebellarWG','frontalAN','temporalAN','parietalAN','cerebellarAN','frontalSM','temporalSM','parietalSM','cerebellarSM')
...
heat.b <- ggplot(data = melted_cormat.b, aes(Var2, Var1, fill = value))+
 geom_tile(color = "white")+
 scale_fill_gradient2(low = "blue", high = "red", mid = "white", 
   midpoint = 0, limit = c(-1,1), space = "Lab", 
   name="Pearson\nCorrelation") +
