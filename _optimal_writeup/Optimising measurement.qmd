...
For LI calculation we departed from the method used by Bruckert (2016) and adopted the Generalised Additive Model (GAM) approach described by @thompson2023, which is more comparable to the Generalised Linear Model method used with fMRI. The new LI~fTCD~ values from the GAM method were used to recategorise individuals as left-lateralised, right-lateralised, or bilateral on each of the two language tasks, depending on whether the 95% confidence interval around the LI estimate was above zero (left), below zero (right), or spanned zero (bilateral).
...
There is, however, another reason why the toolbox LI may give weaker agreement with LI~fTCD~: it ignores task-related deactivation.
...
Locations of data and analysis scripts are specified on Open Science Framework: <https://osf.io/f6uyt/wiki/>.
